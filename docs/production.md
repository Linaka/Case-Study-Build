...
- PDF exports are serialized through a queue and rendered by a separate worker process.
- Tests cover auth, backup behavior, project validation, BD validation, save conflicts and image upload validation.
- GitHub Actions runs install, checks, tests, Markdown export, case-study PDF export and BD PDF export.
...
- PDF export is queued and process-isolated, but still runs on the same host.
